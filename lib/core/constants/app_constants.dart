// Flutter imports:
import 'package:flutter/material.dart';

class AppAssets {
  // Imagens (baseado nos arquivos que você subiu)
  static const String profileImage = 'assets/img/minha_foto.jpg';

  // Documentos
  static const String cvPtBr =
      'assets/docs/curriculo_2024_Franklyn_Roberto.pdf';
}

class AppStrings {
  // Textos Gerais
  static const String portfolioTitle = 'Franklyn Roberto';
  static const String role = 'MOBILE DEVELOPER (FLUTTER) & FULLSTACK';
  static const String aboutMe =
      'Bacharel em Ciência da Computação (UFAL). Desenvolvedor focado em soluções Mobile com Flutter e Backend com Java/Spring Boot. Experiência em aplicações escaláveis, microserviços e arquitetura limpa.';

  // Links Sociais
  static const String linkedInUrl =
      'https://www.linkedin.com/in/franklyn-roberto-dev/';
  static const String gitHubUrl =
      'https://github.com/DevFullStack-Franklyn-R-Silva';
  static const String whatsappUrl =
      'https://api.whatsapp.com/send?phone=5582999915558';
  static const String emailUrl = 'mailto:franklyn.silva@arapiraca.ufal.br';
}

class AppColors {
  // Paleta de cores
  static const int primary = 0xFFBD5D38; // Cor laranja/terrosa
  static const int backgroundLight = 0xFFF9F9F9;
  static const int surfaceWhite = 0xFFFFFFFF;
}
